...
from egait.robots import minitaur
from egait.robots import laikago_motor
from egait.robots import robot_config
from egait.envs import locomotion_gym_config
...
HIP_OFFSETS = np.array([[0.183, -0.00, 0.], [0.183, 0.0, 0.],
...
def foot_position_in_hip_frame_to_joint_angle(foot_position, l_hip_sign=1):
  ...
  angle =  (x**2 + y**2 + z**2 - l_hip**2 - l_low**2 - l_up**2) /(2 * l_low * l_up)
  theta_knee = -np.arccos(angle)
...
class A1(minitaur.Minitaur):
  ...
  def __init__(
    ...
    super(A1, self).__init__(
        ...
        motor_control_mode=_CONTROL_MODE,
        motor_model_class=motor_model_class,
        sensors=sensors,
        motor_kp=motor_kp,
        motor_kd=motor_kd,
        #motor_model_class=lambda: ScaledLaikagoMotorModel(action_scale=action_scale), 
