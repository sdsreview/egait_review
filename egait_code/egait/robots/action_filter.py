...
class ActionFilterButter(ActionFilter):
  ...
  def butter_filter(self, lowcut, highcut, fs, order=5):
    ...
    high = highcut / nyq
    if low:
      # print('WARNING!!! the filter action function has been manually disabled for compatibility issue between scipy and the current ubuntu version (glibc not found)')	
