...
import re
import numpy as np
from egait.robots import minitaur_constants
from egait.robots import minitaur_motor
from egait.robots import robot_config
from egait.robots import action_filter
from egait.robots import kinematics
...
SENSOR_NOISE_STDDEV = (0.001, 0.001, 0.01, 0.01, 0.01)
...
class Minitaur(object):
  ...
  def Step(self, action, control_mode=None,first_step=True):
    """Steps simulation."""
    if self._enable_action_filter:
  ...
  def GetTrueBaseRollPitchYawRate(self):
    ...
    orientation = self.GetTrueBaseOrientation()
    rpy = self.TransformAngularVelocityToLocalFrame(angular_velocity,
                                                     orientation)
  ...
  def ReceiveObservation(self):
    ...
    obs_isaac = [0,] * 43
    obs_isaac[0:12] = self._reorderValuesPybulletIsaac(self.GetTrueObservation()[0:12])
    obs_isaac[12:24] = self._reorderValuesPybulletIsaac(self.GetTrueObservation()[12:24])
    obs_isaac[30:42] = self._reorderValuesPybulletIsaac(self.GetTrueObservation()[30:42])

    self._save_obs_isaac.append(obs_isaac)
  ...
  def ProcessAction(self, action, substep_count):
    ...
    if self._enable_action_interpolation and self._last_action is not None:
      t = float(substep_count + 1) / self._action_repeat
      lerp = self.ease_in_out_cubic(t)
  ...
  def initialize_filter(self,alpha=0.1, num_joints=12):
    # Initialize the state of the filter
    filtered_values = self._last_action
    return alpha, filtered_values

  def apply_filter(self,actions, alpha, filtered_values):
      # Apply the low-pass filter to the actions
      filtered_values = alpha * actions + (1 - alpha) * filtered_values
