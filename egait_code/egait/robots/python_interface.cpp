...
{
public:
    RobotInterface(unit) : safe(LeggedType::A1), udp(level, 8080, "
    192.168.123.15"){
        // InitEnvironment();
    }
...
PYBIND11_MODULE(robot_interface, m) {
    ...
    py::class_<UDP>(m, "UDP")
        .def(py::init<>())
        .def_readwrite("TotalCount", &UDP::TotalCount)
        .def_readwrite("SendCount", &UDP::SendCount)
        .def_readwrite("RecvCount", &UDP::RecvCount)
        .def_readwrite("SendError", &UDP::SendError)
        .def_readwrite("FlagError", &UDP::FlagError)
        .def_readwrite("RecvCRCError", &UDP::RecvCRCError)
        .def_readwrite("RecvLoseError", &UDP::RecvLoseError);
