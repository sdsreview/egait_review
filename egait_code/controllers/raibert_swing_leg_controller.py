...
from controllers import leg_controller
...
class RaibertSwingLegController(leg_controller.LegController):
  ...
  def __init__(
    ...
    self.reset(0)
    self._control_mode = robot._motor_control_mode
  ...
  def get_action(self) -> Mapping[Any, Any]:
    ...
    for joint_id, joint_angle_leg_id in self._joint_angles.items():
      ...
      if self._gait_generator.desired_leg_state[
        ...
        if str(self._control_mode) == 'MotorControlMode.HYBRID':
          action[joint_id] = (joint_angle_leg_id[0], kps[joint_id], 0,
                              kds[joint_id], 0)
          print('IN STANCE HYBRID')
        else:
          action[joint_id] = (joint_angle_leg_id[0])

    return action, self.joint_angles
