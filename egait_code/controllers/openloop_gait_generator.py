...
from controllers import gait_generator
...
class OpenloopGaitGenerator(gait_generator.GaitGenerator):
  ...
  def __init__(
    ...
    self.gait =[]
    self.gait_phase =[]
    self.full_cycle_period = None
    self.full_cycle_periods = []
  ...
  def desired_leg_state(self, value: Sequence[gait_generator.LegState]):
      self._desired_leg_state = value
  ...
  @leg_state.setter
  def leg_state(self, value: Sequence[gait_generator.LegState]):
      self._leg_state = value
  ...
  @stance_duration.setter
  def stance_duration(self, value: Sequence[float]):
      self._stance_duration = value
  ...
  def update(self, current_time):
    ...
    for leg_id in range(self._robot.num_legs):
      ...
      self.full_cycle_period = (self._stance_duration[leg_id] /
      ...
      augmented_time = current_time + self._initial_leg_phase[
          leg_id] * self.full_cycle_period
      phase_in_full_cycle = math.fmod(augmented_time,
                                      self.full_cycle_period) / self.full_cycle_period
      ...
      elif phase_in_full_cycle > ratio:
      ...
      # elif self.gait_selector:
      #   self._desired_leg_state= self._desired_leg_state

  

      self._leg_state[leg_id] = self._desired_leg_state[leg_id]
      phase= self._normalized_phase[leg_id]
      self.gait_phase.append(phase)
      self.gait.append(self._leg_state[leg_id])
      ...
      self.full_cycle_periods.append(self.full_cycle_period)
