...
class LocomotionController(object):
  ...
  def __init__(
    ...
    self._control_mode = str(robot._motor_control_mode)
  ...
  def get_action(self):
    ...
    for joint_id in range(self._robot.num_motors):
      if str(self._control_mode) == 'MotorControlMode.HYBRID':
        ...
        if joint_id in swing_action:
            action.append(swing_action[joint_id],)
        else:
            assert joint_id in stance_action
            action.append(stance_action[joint_id],)

    action = np.array(action, dtype=np.float32)

    return action, dict(qp_sol=qp_sol),joint_pos
